...
async def back_to_menu(callback: types.CallbackQuery):
    ...
    try:
        ...
        msg = await callback.message.answer(text, reply_markup=main_keyboard())
        await save_main_message(user_id, msg.message_id)
    await callback.answer()
...
async def traffic_stats(message: types.Message):
    ...
    if user_id in ADMIN_IDS:
        ...
        text = (f"📊 Ваша статистика трафика\n\n"
...
async def show_tariffs(message: types.Message):
    ...
    try:
        ...
        msg = await message.answer(text, reply_markup=tariffs_keyboard())
        await save_temp_message(user_id, msg.message_id)
